...
(`eval/preferences` is gitignored).
...
```json
{
  ...
  "model_a": "heart-optimized/model.mpd",
  "model_b": "heart-beauty/model.mpd",
  "sha256_a": "…",
  "sha256_b": "…",
  "views": ["front", "iso", "top"],
  "winner": "a",
...
- Models are identified by content (`sha256_*` of the resolved model file);
  paths are context for humans and may go stale.
- `judge` is `claude` or `human`; `confidence` is `high` or `low`. A `claude`
  ...
  lands. Rows are append-only: a human verdict never rewrites the Claude row,
  it supersedes it (the fit prefers `human` rows, then `high`-confidence
  `claude` rows).
