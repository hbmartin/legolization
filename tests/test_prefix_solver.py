...
_RESCUE_WARM = SolverConfig(
    engine="highspy", rescue_warm=True, rescue_direct_min_bricks=1
)


def test_warm_removal_walk_matches_cold():
    layout, ids = _tower_layout()
    solver = RemovalSolver.create(layout, frozenset(ids), _RESCUE_WARM)
    assert solver is not None
    for chunk in ((), (ids[4],), (ids[3], ids[4])):
        warm = solver.probe_without(chunk)
        cold = analyze(layout.subset(set(ids) - set(chunk)), _WARM)
        assert warm.stable == cold.stable
        assert _score_drift(warm, cold) < 1e-9
        solver.commit_without(chunk) if chunk else None


def test_warm_removal_reactivation_restores_state():
    # Probe A, probe B (rolls A back), probe A again: bound flips must
    # leave no residue.
    layout, ids = _tower_layout()
    solver = RemovalSolver.create(layout, frozenset(ids), _RESCUE_WARM)
    assert solver is not None
    first = solver.probe_without((ids[4],))
    solver.probe_without((ids[3],))
    again = solver.probe_without((ids[4],))
    assert _score_drift(first, again) < 1e-12


def test_warm_removal_failure_falls_back_to_components(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import legolization.stability.prefix as prefix_mod
    ...
    class BrokenCore:
        def __init__(self, *_args: object, **_kwargs: object) -> None:
            msg = "warm core exploded"
            raise RuntimeError(msg)

    monkeypatch.setattr(prefix_mod, "_WarmRemovalCore", BrokenCore)
    layout, ids = _tower_layout()
    solver = RemovalSolver.create(layout, frozenset(ids), _RESCUE_WARM)
    assert solver is not None
    with telemetry.record() as session:
    ...
    cold = analyze(layout, _WARM)
    assert result.stable == cold.stable
    assert session.spans["stability.rescue.warm_fail"].calls == 1
    # dead after failure: no second warm attempt
    with telemetry.record() as session:
        solver.probe_without((ids[4],))
    assert "stability.rescue.warm_fail" not in session.spans


def test_rescue_warm_plans_match_default(bad_bridge):
    # The rescue path itself: plans under rescue_warm must be identical
    # in ordering and verdicts to the default engine's plans.
    ...
    layout, _ = bad_bridge
    base_config = InstructionsConfig(
        rotstep=False, solver=SolverConfig(engine="highspy")
    )
    warm_config = InstructionsConfig(
        rotstep=False,
        solver=SolverConfig(
            engine="highspy", rescue_warm=True, rescue_direct_min_bricks=1
        ),
    )
    base = plan_instructions(layout, config=base_config)
    warm = plan_instructions(layout, config=warm_config)
    assert [s.brick_ids for s in base.steps] == [s.brick_ids for s in warm.steps]
    assert [s.prefix_stable for s in base.steps] == [
        s.prefix_stable for s in warm.steps
    ]
    assert verify_plan(layout, warm, config=warm_config) == []
